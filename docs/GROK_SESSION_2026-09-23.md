# Grok session — 23 September 2026
...
rehearsal station, module tools, a personal ISO bake checklist, an
independent download URL, and an xAI handoff packet.
...
- Overlay version bump to **1.11.0**. Official Zen Garden flavor identity
  brought in line with the overlay (was lagging at 1.9.0).
- `experimentals/handoff.json` — single wiring packet for a future Grok
  App Experimentals tab.
- `harborctl app-contract` / `harborctl handoff` print that packet.
- Overlay pack now writes `SHA256SUMS.txt` and `checksums.json`.
- Experimentals local station rail now matches the docs station:
  Experimentals → Harbor OS / Flavors / Upload flavor (Premium+).
- Manifest / catalog / client-ia dates synced to 2026-09-23.
...
| Handoff | experimentals/handoff.json (after merge) |
