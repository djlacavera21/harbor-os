# Grok session — 24 September 2026
...
- Overlay version bump to **1.11.0**.
- Self-contained Experimentals station (`docs/index.html`) so the
  independent htmlpreview link renders without sibling CSS/JS.
- Rail submenu wiring: Experimentals → Harbor OS / Flavors / Upload flavor.
- Upload flavor remains locked until the local Premium+ gate opens.
- Flavor search and "Copy download link" actually work.
- `harborctl app-contract` now exists (promised in 1.10.0, missing from the script).
- Catalog, manifest, client IA, and official Zen Garden flavor identity at 1.11.0.
